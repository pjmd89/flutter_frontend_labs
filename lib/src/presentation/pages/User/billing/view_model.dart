...
class ViewModel extends ChangeNotifier {
  ...
  // Query para obtener Invoices filtrados por laboratory.company.owner._id
  ...
  Future<void> loadInvoices() async {
    ...
    try {
      ...
      if (response is EdgeInvoice) {
        ...
        // Filtrar en el frontend por laboratory.company.owner.id = userId
        _invoiceList = allInvoices.where((invoice) {
          final ownerMatches = invoice.laboratory?.company?.owner?.id == userId;
          debugPrint('   Invoice: ${invoice.orderID} - Owner: ${invoice.laboratory?.company?.owner?.id} - Match: $ownerMatches');
          debugPrint('   Laboratory Address: ${invoice.laboratory?.address}');
          return ownerMatches;
        }).toList();
        
        debugPrint('✅ Encontrados ${_invoiceList?.length ?? 0} invoices filtrados para userId: $userId');
        _loading = false;
        notifyListeners();
      }
    } catch (e, stackTrace) {
